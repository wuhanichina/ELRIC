function [n_old_ELRIC, n_new_ELRIC, ELRIC] = ELRIC_Gen_fun(case_name,gen,eta,r,d,Asset,C_max)
...
for i = 1:length(P_i)
    ...
    for l=setdiff(1:length(P_l),find(P_l==0)) % 线路14上没有功率。。。所以这里就略掉吧
        alpha(l) = abs(H(l,i));% 注意每换一个注入节点，这两个参数都要重新计算
        beta(l) = abs(H(l,setdiff(1:length(P_i),i))*MPC_case.bus(setdiff(1:length(P_i),i),3)); % Pbus就不需要调整节点编号
        ELRIC_get_n = @(n)new_cvar_equal(n, delta_Pi,d, C_max, Asset, eta, r, alpha(l), beta(l), q_up_load, pd_load_up_s, pd_load_up_k);        interval = [1  100];
